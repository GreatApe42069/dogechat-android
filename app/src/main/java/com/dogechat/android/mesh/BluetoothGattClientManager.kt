...
import com.dogechat.android.ui.debug.DebugSettingsManager
import com.dogechat.android.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    companion object {
        ...
        private const val RSSI_UPDATE_INTERVAL = com.dogechat.android.util.AppConstants.Mesh.RSSI_UPDATE_INTERVAL_MS // 5 seconds
    }
    ...
    fun start(): Boolean {
        ...
        try {
            if (!com.dogechat.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value) {
    ...
    fun onScanStateChanged(shouldScan: Boolean) {
        val enabled = try { com.dogechat.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
    ...
    private fun connectToDevice(device: BluetoothDevice, rssi: Int) {
        ...
        val deviceAddress = device.address
        Log.i(TAG, "Connecting to dogechat device: $deviceAddress")
