...
// Do NOT import Tink directly here — use reflection for XChaCha20Poly1305 so compilation doesn't require Tink.
...
object NostrCrypto {
    ...
    private fun computeSharedPointWithParity(privateKeyHex: String, publicKeyHex: String, preferOddY: Boolean): ECPoint {
        val privateKeyBytes = privateKeyHex.hexToByteArray()
        val privateKeyBigInt = BigInteger(1, privateKeyBytes)
        val point = recoverPublicKeyPointWithParity(publicKeyHex.hexToByteArray(), preferOddY)
     ...
     *
     * Uses reflection to call Tink's XChaCha20Poly1305 if available on the runtime classpath.
     */
    fun encryptNIP44(
        plaintext: String,
        ...
        try {
            // derive HKDF input from the compressed shared point (33 bytes)
            ...
            val combined = aeadEncryptWithReflection(encryptionKey, plaintext.toByteArray(Charsets.UTF_8))
        ...
        try {
            ...
            for (preferOdd in listOf(false, true)) {
                try {
                    ...
                    val pt = aeadDecryptWithReflection(key, encryptedData)
    ...
    /**
     * Reflection-based wrapper that attempts to instantiate com.google.crypto.tink.subtle.XChaCha20Poly1305
     * and call encrypt(plaintext, associatedData) -> ByteArray.
     *
     * If the class is not present at runtime, throws RuntimeException with a clear message.
     */
    private fun aeadEncryptWithReflection(key: ByteArray, plaintext: ByteArray): ByteArray {
        try {
            val cls = Class.forName("com.google.crypto.tink.subtle.XChaCha20Poly1305")
            val ctor = cls.getConstructor(ByteArray::class.java)
            val instance = ctor.newInstance(key)
            val encryptMeth = cls.getMethod("encrypt", ByteArray::class.java, ByteArray::class.java)
            // call encrypt(plaintext, associatedData = null)
            return encryptMeth.invoke(instance, plaintext, null) as ByteArray
        } catch (cnfe: ClassNotFoundException) {
            throw RuntimeException("XChaCha20Poly1305 class not found. Add Tink or another XChaCha20-Poly1305 implementation to runtime classpath.", cnfe)
        } catch (e: NoSuchMethodException) {
            throw RuntimeException("Unexpected XChaCha20Poly1305 API shape on classpath.", e)
        } catch (e: Exception) {
            throw RuntimeException("AEAD encrypt failed via reflection: ${e.message}", e)
        }
    }

    private fun aeadDecryptWithReflection(key: ByteArray, ciphertext: ByteArray): ByteArray {
        try {
            val cls = Class.forName("com.google.crypto.tink.subtle.XChaCha20Poly1305")
            val ctor = cls.getConstructor(ByteArray::class.java)
            val instance = ctor.newInstance(key)
            val decryptMeth = cls.getMethod("decrypt", ByteArray::class.java, ByteArray::class.java)
            // call decrypt(ciphertext, associatedData = null)
            return decryptMeth.invoke(instance, ciphertext, null) as ByteArray
        } catch (cnfe: ClassNotFoundException) {
            throw RuntimeException("XChaCha20Poly1305 class not found. Add Tink or another XChaCha20-Poly1305 implementation to runtime classpath.", cnfe)
        } catch (e: NoSuchMethodException) {
            throw RuntimeException("Unexpected XChaCha20Poly1305 API shape on classpath.", e)
        } catch (e: Exception) {
            throw RuntimeException("AEAD decrypt failed via reflection: ${e.message}", e)
        }
    }
    ...
    // ====================================================================
    // BIP-340 Schnorr signatures (full implementation)
    // ====================================================================
    ...
    private fun hasEvenY(point: ECPoint): Boolean {
        ...
        // yCoord is big-endian; parity is lowest-order bit of last byte
        return (yCoord[yCoord.size - 1].toInt() and 1) == 0
    }

    /**
     * Lift x coordinate to point with even y (if possible)
     */
    private fun liftX(xBytes: ByteArray): ECPoint? {
        return try {
            val point = recoverPublicKeyPoint(xBytes)
            val normalizedPoint = point.normalize()
            if (hasEvenY(normalizedPoint)) normalizedPoint else normalizedPoint.negate()
        } catch (e: Exception) {
            null
        }
    }
     ...
     *
     * NOTE: This implementation uses a simple nonce generation approach.
     * For production, RFC6979 deterministic nonce generation or an approved library should be used.
     */
