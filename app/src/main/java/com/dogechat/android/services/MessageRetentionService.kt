 ...
 * 
 * Uses JSON serialization with Gson instead of Java ObjectOutputStream
...
class MessageRetentionService private constructor(private val context: Context) {
    ...
    fun toggleFavoriteChannel(channel: String): Boolean {
        ...
        if (!wasAdded) {
            // Channel removed from favorites - delete saved messages in background using coroutines
    ...
    private fun getChannelFile(channel: String): File {
        ...
        return File(retentionDir, "channel_${sanitizedChannel}.json")
    }
    ...
    private fun loadMessagesFromFile(file: File): List<DogechatMessage> {
        ...
        return try {
            ...
            val type = object : TypeToken<List<DogechatMessage>>() {}.type
            gson.fromJson<List<DogechatMessage>>(json, type) ?: emptyList()
        } catch (e: Exception) {
    ...
    suspend fun getTotalStoredMessagesCount(): Int = withContext(Dispatchers.IO) {
        ...
        try {
            retentionDir.listFiles()?.forEach { file ->
                if (file.name.startsWith("channel_") && file.name.endsWith(".json")) {
