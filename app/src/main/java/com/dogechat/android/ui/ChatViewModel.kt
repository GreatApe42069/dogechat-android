...
class ChatViewModel(
    ...
    val messages: StateFlow<List<DogechatMessage>> = state.messages
    ...
    val privateChats: StateFlow<Map<String, List<DogechatMessage>>> = state.privateChats
    ...
    val channelMessages: StateFlow<Map<String, List<DogechatMessage>>> = state.channelMessages
    ...
    init {
        ...
        viewModelScope.launch {
            try { com.dogechat.android.services.AppStateStore.peers.collect { peers ->
                state.setConnectedPeers(peers)
                state.setIsConnected(peers.isNotEmpty())
            } } catch (_: Exception) { }
        }
        viewModelScope.launch {
            try { com.dogechat.android.services.AppStateStore.publicMessages.collect { msgs ->
                // Source of truth is AppStateStore; replace to avoid duplicate keys in LazyColumn
                state.setMessages(msgs)
            } } catch (_: Exception) { }
        }
        viewModelScope.launch {
            try { com.dogechat.android.services.AppStateStore.privateMessages.collect { byPeer ->
                ...
                try {
                    val seen = com.dogechat.android.services.SeenMessageStore.getInstance(getApplication())
        ...
        viewModelScope.launch {
            try { com.dogechat.android.services.AppStateStore.channelMessages.collect { byChannel ->
                // Replace with store snapshot
    ...
    fun sendMessage(content: String) {
        ...
        if (selectedPeer != null) {
            ...
            privateChatManager.sendPrivateMessage(
                ...
                val router = com.dogechat.android.services.MessageRouter.getInstance(getApplication(), meshService)
