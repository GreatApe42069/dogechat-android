...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = com.dogechat.android.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = com.dogechat.android.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    ...
    fun addMessage(message: DogechatMessage) {
        ...
        try { com.dogechat.android.services.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
    }
    ...
    fun addChannelMessage(channel: String, message: DogechatMessage) {
        ...
        try { com.dogechat.android.services.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
    ...
    fun addPrivateMessage(peerID: String, message: DogechatMessage) {
        ...
        // Reflect into process-wide store
        try { com.dogechat.android.services.AppStateStore.addPrivateMessage(peerID, message) } catch (_: Exception) { }
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { com.dogechat.android.services.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
