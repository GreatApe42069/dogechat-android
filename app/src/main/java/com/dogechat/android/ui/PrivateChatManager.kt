...
class PrivateChatManager(
    ...
    fun handleIncomingPrivateMessage(message: DogechatMessage, suppressUnread: Boolean) {
    ...
    fun sendReadReceiptsForPeer(peerID: String, meshService: BluetoothMeshService) {
        ...
        val chats = try { state.getPrivateChatsValue() } catch (_: Exception) { emptyMap<String, List<DogechatMessage>>() }
    ...
    private fun consolidateNostrTempConversationIfNeeded(targetPeerID: String) {
        ...
        try {
            ...
            val npub = com.dogechat.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKeyBytes)
            if (npub != null) {
