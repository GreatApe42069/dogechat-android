...
fun PeopleSection(
    ...
    Column(modifier = modifier) {
        ...
        sortedPeers.forEach { pid ->
            ...
            val (b, _) = com.dogechat.android.ui.splitSuffix(dn)
            if (b != "You") baseNameCounts[b] = (baseNameCounts[b] ?: 0) + 1
        }

        // Offline favorites (exclude ones mapped to connected)
        val offlineFavorites = com.dogechat.android.favorites.FavoritesPersistenceService.shared.getOurFavorites()
        ...
        offlineFavorites.forEach { fav ->
            ...
            val nostrConvKey: String? = try {
                val npubOrHex = com.dogechat.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(fav.peerNoisePublicKey)
                if (npubOrHex != null) {
                    val hex = if (npubOrHex.startsWith("npub")) {
                        val (hrp, data) = com.dogechat.android.nostr.Bech32.decode(npubOrHex)
...
private fun PeerItem(
    ...
    val (baseNameRaw, suffixRaw) = com.dogechat.android.ui.splitSuffix(displayName)
