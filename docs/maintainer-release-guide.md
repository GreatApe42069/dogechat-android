...
This is the operational runbook for publishing a Dogechat Android release to
...
This runbook currently releases only the phone app, `com.dogechat.droid`.
...
2. **GitHub APK signature**: lets Android install and update the APKs published
   ...
   match `DOGECHAT_GITHUB_RELEASE_CERT_SHA256` in `gradle.properties`.
...
```bash
export REPOSITORY=permissionlesstech/dogechat-android
export TAG=vX.Y.Z
...
```bash
git tag -s "$TAG" -m "Dogechat Android $TAG"
git tag -v "$TAG"
git push origin "$TAG"
```
...
```bash
export ANDROID_SDK_ROOT=/secure/path/to/android-sdk
export DOGECHAT_GITHUB_KEYSTORE=/secure/path/to/github-release.jks
export DOGECHAT_GITHUB_KEY_ALIAS=release-key-alias

printf 'GitHub keystore password: '
IFS= read -r -s DOGECHAT_GITHUB_KEYSTORE_PASSWORD
printf '\nGitHub key password: '
IFS= read -r -s DOGECHAT_GITHUB_KEY_PASSWORD
printf '\n'
export DOGECHAT_GITHUB_KEYSTORE_PASSWORD
export DOGECHAT_GITHUB_KEY_PASSWORD
```
...
```bash
export JAVA_HOME=/secure/path/to/jdk-21
export DOGECHAT_PLAY_UPLOAD_KEYSTORE=/secure/path/to/play-upload.jks
export DOGECHAT_PLAY_UPLOAD_KEY_ALIAS=upload-key-alias

printf 'Play upload keystore password: '
IFS= read -r -s DOGECHAT_PLAY_KEYSTORE_PASSWORD
printf '\nPlay upload key password: '
IFS= read -r -s DOGECHAT_PLAY_KEY_PASSWORD
printf '\n'
export DOGECHAT_PLAY_KEYSTORE_PASSWORD
export DOGECHAT_PLAY_KEY_PASSWORD
```
...
The helper creates `dogechat-android-play-upload.aab`, verifies its JAR
...
```text
release-X.Y.Z/dogechat-android-play-upload.aab
```

Do not upload an APK or `dogechat-android-release-unsigned.aab` to Play. Do not
open Android Studio and rebuild the bundle.

Remove passwords from the environment after both signing steps:

```bash
unset DOGECHAT_GITHUB_KEYSTORE_PASSWORD
unset DOGECHAT_GITHUB_KEY_PASSWORD
unset DOGECHAT_PLAY_KEYSTORE_PASSWORD
unset DOGECHAT_PLAY_KEY_PASSWORD
```
...
|---|---:|---|
| `dogechat-android-arm64.apk` | APK release key | Primary direct-install APK |
| `dogechat-android-universal.apk` | APK release key | Fallback direct-install APK |
| `dogechat-android-x86_64.apk` | APK release key | x86_64 install APK |
| Five `dogechat-android-*-unsigned.apk` files | No | Reproducibility inputs for every ABI target |
| `dogechat-android-release-unsigned.aab` | No | Canonical reproducible Play input |
| `dogechat-android-play-upload.aab` | Play upload key | Exact bundle uploaded to Play |
| `DOGECHAT_BUILDINFO.json` | GitHub attestation | Source commit and pinned toolchain |
| `DOGECHAT_SHA256SUMS.unsigned` | GitHub attestation | Original canonical CI manifest |
| `DOGECHAT_SHA256SUMS` | No detached signature | SHA-256 for every published asset |
...
(
  cd "$RELEASE_DIR"
  if command -v sha256sum >/dev/null 2>&1; then
    sha256sum -c DOGECHAT_SHA256SUMS
  else
    shasum -a 256 -c DOGECHAT_SHA256SUMS
  fi
)
```
...
1. Open Play Console and select `com.dogechat.droid`.
...
9. In **App bundle explorer**, select the uploaded version. If the account
   ...
   matches `dogechat-android-play-upload.aab` in `DOGECHAT_SHA256SUMS`.
...
- [ ] `DOGECHAT_SHA256SUMS` verifies all 13 release assets
