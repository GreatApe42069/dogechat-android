...
Dogechat's canonical phone-app release build produces byte-for-byte reproducible
...
```bash
git clone https://github.com/permissionlesstech/dogechat-android.git
cd dogechat-android
git checkout vX.Y.Z
...
```bash
DOGECHAT_CONTAINER_GRADLE_HOME_NAME=gradle-home-first \
  tools/reproducible-builds/build-in-container.sh .reproducible-build/first
DOGECHAT_CONTAINER_GRADLE_HOME_NAME=gradle-home-second \
...
3. verifies `DOGECHAT_SHA256SUMS`;
...
```bash
apksigner verify --verbose --print-certs dogechat-android-universal.apk
```

Compare the reported signer certificate SHA-256 with
`DOGECHAT_GITHUB_RELEASE_CERT_SHA256` in `gradle.properties`. A matching
...
tools/reproducible-builds/compare-archive-payloads.sh \
  .reproducible-build/local-vX.Y.Z/dogechat-android-universal-unsigned.apk \
...
gh attestation verify dogechat-android-universal-unsigned.apk \
  --repo permissionlesstech/dogechat-android
```
...
1. In Play Console, open **Test and release > App bundle explorer**, select the
   ...
   ```bash
   tools/reproducible-builds/compare-archive-payloads.sh \
     .reproducible-build/local-vX.Y.Z/dogechat-android-release-unsigned.aab \
...
4. Confirm package name `com.dogechat.droid`, version code, version name, and
...
5. Recreate Google's split-generation behavior from the reproduced AAB with the
   ...
   bundletool build-apks \
     --bundle=dogechat-android-release-unsigned.aab \
     --output=local.apks \
...
locally creates `dogechat-android-play-upload.aab` from that exact file and
