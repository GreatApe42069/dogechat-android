...
This document specifies the gossip-based synchronization feature for DogeChat, inspired by Plumtree. It ensures eventual consistency of public packets (ANNOUNCE and broadcast MESSAGE) across nodes via periodic sync requests containing a compact Golomb‑Coded Set (GCS) of recently seen packets.

## Overview

- Each node maintains a rolling set of public DogeChat packets it has seen recently:
...
Implementation: `com.dogechat.android.sync.PacketIdUtil`.

## GCS Filter (On-demand)

Implementation: `com.dogechat.android.sync.GCSFilter`.
...
- Header: normal DogeChat header with TTL indicating “local-only” semantics. Implementations SHOULD set TTL=0 to prevent any relay; neighbors still receive the packet over the direct link-layer. For periodic sync, recipient is broadcast; for per-peer initial sync, recipient is the specific peer.
...
Encode/Decode implementation: `com.dogechat.android.model.RequestSyncPacket`.
