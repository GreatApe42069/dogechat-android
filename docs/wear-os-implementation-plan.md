# Dogechat for Pixel Watch — Implementation Plan
...
- **Reference app**: this repository (`dogechat-android`) is a fully working, decentralized BLE mesh
  chat client. Single Gradle module `:app`, root package `com.dogechat.android`, applicationId
  `com.dogechat.droid`. See `AGENTS.md` for the full architecture overview.
- **Goal**: a new `:wear` Gradle module (applicationId `com.dogechat.watch`) — a standalone Wear OS
  app for the Pixel Watch. **Bluetooth mesh only**: global chat, Noise-encrypted direct messages,
  and receiving/displaying files & images. It must be a fully interoperable dogechat client: scan,
...
the dogechat protocol stack compiles unmodified.
...
- `model/**` — `DogechatMessage`, `DogechatFilePacket`, `FragmentPayload`, `NoiseEncrypted`,
...
(wear gets its own service), `DogechatApplication.kt`, `MainActivity.kt`.
...
- **Visual identity** (mirrors `ui/theme/` exactly): black background `#000000`, green primary
  ...
  (`PeerColors.kt` algorithm), Geist Mono typography, `DogechatMotion` timing tokens
...
- Two phones running the Android dogechat app, also on ADB, for interop testing (used heavily from
...
"dogechat" wordmark (green `#32D74B`, Geist Mono, black background) correctly centered on the
...
references from shared code: `com.dogechat.android.service.MeshServiceHolder` (BLE-toggle
interface, null) and `com.dogechat.android.wifiaware.WifiAwareController` (no-op).
...
**Success criteria**: the phone's dogechat app lists the watch as a connected peer and vice versa
...
- [ ] Send/receive/relay public `DogechatMessage`s (relay/TTL comes free from shared mesh code)
- [ ] Chat timeline UI (`ScalingLazyColumn`, message bubbles per dogechat style, peer colors,
...
- [x] Receive broadcast + Noise-encrypted private files (shared `DogechatFilePacket` TLV,
...
- [x] Send as `DogechatFilePacket` broadcast in global chat (`MeshCore.sendFileBroadcast`); in a
...
- [x] Received voice notes (`DogechatMessageType.Audio`, `content` = local path) render as a
...
- [x] Wear debug-only `TestHookReceiver` (`wear/src/debug/`) mirroring the phone's command set:
  ...
  `clear_results` — broadcast action `com.dogechat.watch.TEST_HOOK`, same JSON-result-file protocol
...
in `mesh_lab.py`: `WatchDevice` (package/hook/permissions/activity for `com.dogechat.watch`),
...
- [x] Animations/transitions per `DogechatMotion` tokens; message-appear animations; screen
...
adb -s <watch-serial> shell monkey -p com.dogechat.watch -c android.intent.category.LAUNCHER 1
...
adb -s <watch-serial> shell am broadcast -a com.dogechat.watch.TEST_HOOK \
  -n com.dogechat.watch/.testhook.WearTestHookReceiver --es cmd state --es id s1
adb -s <watch-serial> shell run-as com.dogechat.watch cat cache/testhook/results/s1.json
```
