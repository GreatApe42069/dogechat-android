...
: "${DOGECHAT_GITHUB_KEYSTORE:?DOGECHAT_GITHUB_KEYSTORE is required}"
: "${DOGECHAT_GITHUB_KEY_ALIAS:?DOGECHAT_GITHUB_KEY_ALIAS is required}"
: "${DOGECHAT_GITHUB_KEYSTORE_PASSWORD:?DOGECHAT_GITHUB_KEYSTORE_PASSWORD is required}"
: "${DOGECHAT_GITHUB_KEY_PASSWORD:?DOGECHAT_GITHUB_KEY_PASSWORD is required}"

if [ ! -f "$DOGECHAT_GITHUB_KEYSTORE" ]; then
  echo "error: GitHub release keystore not found" >&2
  exit 1
fi
KEYSTORE="$(
  cd "$(dirname "$DOGECHAT_GITHUB_KEYSTORE")"
  printf '%s/%s\n' "$PWD" "$(basename "$DOGECHAT_GITHUB_KEYSTORE")"
)"
...
EXPECTED_CERT_SHA256="$(
  sed -n 's/^DOGECHAT_GITHUB_RELEASE_CERT_SHA256=//p' "$PROJECT_ROOT/gradle.properties" |
    tr -d ':\r[:space:]' |
    tr '[:upper:]' '[:lower:]'
)"
...
for ((index = 0; index < ${#unsigned_names[@]}; index++)); do
  ...
  for output_apk in "$first_signed" "$second_signed"; do
    "$APKSIGNER" sign \
      --ks "$KEYSTORE" \
      --ks-key-alias "$DOGECHAT_GITHUB_KEY_ALIAS" \
      --ks-pass env:DOGECHAT_GITHUB_KEYSTORE_PASSWORD \
      --key-pass env:DOGECHAT_GITHUB_KEY_PASSWORD \
