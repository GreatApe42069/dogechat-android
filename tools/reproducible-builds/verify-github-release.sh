...
MODE="${2:-}"
REPOSITORY="${DOGECHAT_GITHUB_REPOSITORY:-greatape42069/dogechat-android}"
TEMP_DIR="$(mktemp -d)"
...
gh release download "$TAG" \
  --repo "$REPOSITORY" \
  --dir "$DOWNLOAD_DIR" \
  --pattern 'DOGECHAT_BUILDINFO.json' \
  --pattern 'DOGECHAT_SHA256SUMS' \
  --pattern 'DOGECHAT_SHA256SUMS.unsigned' \
  --pattern 'dogechat-android-*.apk' \
  --pattern 'dogechat-android-*.aab'

attested_artifacts=(
  DOGECHAT_BUILDINFO.json
  DOGECHAT_SHA256SUMS.unsigned
...
mv "$DOWNLOAD_DIR/DOGECHAT_BUILDINFO.json" "$DOWNLOAD_DIR/BUILDINFO.json"
mv "$DOWNLOAD_DIR/DOGECHAT_SHA256SUMS" "$DOWNLOAD_DIR/SHA256SUMS"
mv "$DOWNLOAD_DIR/DOGECHAT_SHA256SUMS.unsigned" "$DOWNLOAD_DIR/SHA256SUMS.unsigned"
