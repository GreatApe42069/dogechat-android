package com.dogechat.watch.mesh

import android.bluetooth.BluetoothDevice
import android.content.Context
import android.util.Log
import com.dogechat.android.crypto.EncryptionService
import com.dogechat.android.favorites.FavoriteControlMessage
import com.dogechat.android.mesh.BluetoothConnectionManager
import com.dogechat.android.mesh.BluetoothConnectionManagerDelegate
import com.dogechat.android.mesh.DirectLinkAnnouncementPolicy
import com.dogechat.android.mesh.MeshCore
import com.dogechat.android.mesh.MeshTransport
...
class WearMeshService private constructor(private val context: Context) {
    ...
    var onPrivateMessage: ((com.dogechat.android.model.DogechatMessage) -> Unit)? = null
    ...
    private inner class BleTransport : MeshTransport {
        ...
        override fun sendPacketToPeer(peerID: String, packet: DogechatPacket): Boolean =
            connectionManager.sendPacketToPeer(peerID, packet)

        override fun sendPacketToLink(
            relayAddress: String,
            ingressLinkID: String,
            packet: DogechatPacket
     ...
     */
    fun sendFilePrivateEncrypted(recipientPeerID: String, file: com.dogechat.android.model.DogechatFilePacket) {
        serviceScope.launch {
            ...
            val transferId = com.dogechat.android.mesh.MeshPacketUtils.sha256Hex(
            ...
            while (System.currentTimeMillis() < prepDeadline) {
                when (val prep = meshCore.prepareFilePrivate(recipientPeerID, file, transferId, allowLegacyFallback = false)) {
                    is com.dogechat.android.mesh.PrivateMediaPreparation.Ready -> {
                        prep.transfer.commit()
                        return@launch
                    }
                    com.dogechat.android.mesh.PrivateMediaPreparation.AwaitingPeerState,
                    com.dogechat.android.mesh.PrivateMediaPreparation.NeedsHandshake -> {
                        if (prep == com.dogechat.android.mesh.PrivateMediaPreparation.NeedsHandshake) {
                            try { initiateNoiseHandshake(recipientPeerID) } catch (_: Exception) { }
                        }
                        delay(500)
                    }
    ...
    private fun prefs() = context.getSharedPreferences("dogechat_watch_prefs", Context.MODE_PRIVATE)
